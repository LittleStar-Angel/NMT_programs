...
#NMT
sys.path.append('../../nematus/nematus/')
...
class NMTService:
    def __init__(self, workers, logger):
        ...
        self.models = ['../../nematus/nematus/nist.model_zhen.npz']
        self.source_file = '../../test/tmp'
        self.saveto = '../../test/tmp.tran.ser'
        self.k = 3
        self.n_process = 1
        ##
        server_translate.Load_Model(self.models, self.source_file, self.saveto, save_alignment=True, k=self.k,normalize=False, n_process=self.n_process, chr_level=False, verbose=False, nbest=False, suppress_unk=False, a_json=False, print_word_probabilities=False, return_hyp_graph=False)
    ...
    def get(self):
        ...
        #tmp = self._dispatch_task(args)
        #return
        #Translating
        #result = self.translate(args['text'],args['alignmentInfo'])
        #result = self.translate(args['text'],args['alignmentInfo'])
        args['alignmentInfo'] = self.S2B(args['alignmentInfo'])
        print args['text']
        result = self.translate(args['text'],args['alignmentInfo'])
